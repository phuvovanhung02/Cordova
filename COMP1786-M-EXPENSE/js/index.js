...
function confirmTrip(e) {
    ...
    $('#page-create #frm-confirm #username').text($('#page-create #frm-register #username').val());
    $('#page-create #frm-confirm #phone').text($('#page-create #frm-register #phone').val());
    ...
    $('#page-create #frm-confirm #number').text($('#page-create #frm-register #number').val());
...
function registerTrip(e) {
    ...
    let username = $('#page-create #frm-register #username').val();
    let phone = $('#page-create #frm-register #phone').val();
    ...
    let number = $('#page-create #frm-register #number').val();
    let risk = $('#page-create #frm-register #risk').val();

    db.transaction(function (tx) {
        let query = `INSERT INTO Trip (Name,Username,Phonenumber, Destination, Description,Number, Risk, Date) VALUES (?,?,?,?,?,?,?,julianday('${date}'))`;
        tx.executeSql(query, [name,username,phone, destination, description,number, risk], transactionSuccess, transactionError);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $('#page-detail #detail #username').text(result.rows[0].Username);
                $('#page-detail #detail #phone').text(result.rows[0].Phonenumber);
                ...
                $('#page-detail #detail #number').text(result.rows[0].Number);
    ...
    db.transaction(function (tx) {
        ...
        function transactionSuccess(tx, result) {
            if (result.rows[0] != null) {
                ...
                $(`#page-detail #frm-update #phone`).val(result.rows[0].Phone);
                ...
                $(`#page-detail #frm-update #number`).val(result.rows[0].Number);
...
function updateTrip(e) {
    ...
    let username = $('#page-detail #frm-update #username').val();
    let phone = $('#page-detail #frm-update #phone').val();
    ...
    let number = $('#page-detail #frm-update #number').val();
    let risk = $('#page-detail #frm-update #risk').val();

    db.transaction(function (tx) {
        let query = `UPDATE Trip SET Name = ?,Username = ?,Phonenumber = ?, Destination = ?, Description = ?,Number = ?, Risk = ?, Date = julianday('${date}') WHERE Id = ?`;

        tx.executeSql(query, [name,username,phone, destination, description,number, risk, id], transactionSuccess, transactionError);
...
function search(e) {
    ...
    let username = $('#page-list #frm-search #username').val();
    let phone = $('#page-list #frm-search #phone').val();
    ...
    db.transaction(function (tx) {
        ...
        query += username ? ` Username LIKE "%${username}%"   AND` : '';
        query += phone ? ` Phonenumber LIKE "%${phone}%"   AND` : '';
